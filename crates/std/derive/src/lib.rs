...
pub fn cw_derive(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let mut derive_prost = false;
    for attr in attrs {
        match attr {
            NestedMeta::Meta(Meta::Path(path)) if path.is_ident("serde") => {
                derive_serde = true;
            },
            NestedMeta::Meta(Meta::Path(path)) if path.is_ident("prost") => {
                derive_prost = true;
            },
            _ => {
                panic!("unsupported attribute, expecting `serde` or `prost`");
            },
        }
    }

    let derives = match (derive_serde, derive_prost) {
        (false, true) => quote! {
            #[derive(
                ::cw_std::__private::prost::Message,
                ::std::clone::Clone,
                ::std::fmt::Debug,
                ::std::cmp::PartialEq,
                ::std::cmp::Eq,
        ...
        _ => {
            panic!("unsupported attribute combination: expecting either `serde`, `prost`, or both");
        },
    };
